...
export function WeddingSettingsEditor({ wedding, onSaved }: WeddingSettingsEditorProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* ── Gift Registry note (shown only when toggle is on; items/bank accounts managed in the Registry section) ── */}
      {config.show_gift_registry && (
        <div className="admin-card p-5">
          <h3 className="text-xs tracking-widest uppercase text-ivory/30 mb-1">Gift Registry</h3>
          <p className="text-[11px] text-ivory/25 mb-5">
            Add a note shown at the top of your registry page. Manage items and bank accounts in the <strong className="text-ivory/40">Gift Registry</strong> section.
          </p>
          <div>
            <label className="rsvp-label">Registry Note (optional)</label>
            <input
              type="text"
              value={(config.gift_registry_note as string) ?? ''}
              onChange={e => setConfigField('gift_registry_note', e.target.value || undefined)}
